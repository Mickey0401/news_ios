...
//  havr
//
//  Created by Ismajl Marevci on 8/24/17.
...
open class Sound {
    ...
    public static var playersPerSound: Int = 1 {
        didSet {
            stopAll()
            sounds.removeAll()
        }
    ...
    public static var category: SoundCategory = {
        ...
        try? session.setCategory(defaultCategory.avFoundationCategory, with: AVAudioSessionCategoryOptions.defaultToSpeaker)
        return defaultCategory
    }() {
        didSet {
            try? session.setCategory(category.avFoundationCategory, with: AVAudioSessionCategoryOptions.defaultToSpeaker)
        }
    }
    #endif
    ...
    public static let stopNotificationName = Notification.Name("com.moonlightapps.SwiftySound.stopNotification")
