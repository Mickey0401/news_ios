...
open class CameraManager: NSObject, AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate {
    ...
    open func capturePictureWithCompletition(_ imageCompletition: @escaping (UIImage?, NSError?) -> Void) {
        self.capturePictureDataWithCompletition { data, error in
            ...
            if self.writeFilesToPhoneLibrary == true, let library = self.library  {
                ...
                library.writeImageData(toSavedPhotosAlbum: imageData, metadata:nil, completionBlock: { picUrl, error in
     ...
     */
    open func stopRecordingVideo(_ completition:@escaping (_ videoURL: URL?, _ error: NSError?) -> Void) {
    ...
    open func capture(_ captureOutput: AVCaptureFileOutput!, didFinishRecordingToOutputFileAt outputFileURL: URL!, fromConnections connections: [Any]!, error: Error!) {
        ...
        if (error != nil) {
            ...
            videoCompletition?(nil, error as NSError)
            
        } else if let url = outputFileURL {
            videoCompletition?(url,nil)
        }
    }
